...
def iter_route_steps(route: Optional[Dict[str, Any]]) -> Iterable[Dict[str, Any]]:
...
def route_has_private_or_construction(route: Optional[Dict[str, Any]]) -> Tuple[bool, str]:
    for step in iter_route_steps(route):
        txt = _step_text(step)
...
def validate_route_for_mode(route: Optional[Dict[str, Any]], mode: Optional[str]) -> Tuple[bool, List[str]]:
    """
...
def filter_routes_for_mode(routes: Optional[List[Dict[str, Any]]], mode: Optional[str]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
...
def is_reasonable_detour(original_km: Optional[Union[float, int]], new_km: Optional[Union[float, int]]) -> bool:
...
def detour_limit_text(original_km: Optional[Union[float, int]]) -> str:
    try:
        old = float(original_km or 0.0)
    except Exception:
...
def is_reasonable_motorbike_avoid_expressway(original_km: Optional[Union[float, int]], new_km: Optional[Union[float, int]]) -> bool:
...
def motorbike_avoid_expressway_limit_text(original_km: Optional[Union[float, int]]) -> str:
    try:
        old = float(original_km or 0.0)
    except Exception:
        return "không quá xa phi lý"
...
def _route_distance_km_safe(route: Optional[Dict[str, Any]]) -> float:
    try:
        if not route:
            return 0.0
...
def _add_candidate(candidates: List[Dict[str, Any]], route: Optional[Dict[str, Any]], source: str) -> None:
...
def _combine_route_parts(parts: List[Dict[str, Any]], source: str = "Motorbike corridor") -> Optional[Dict[str, Any]]:
...
# ─────────────────────────────────────────────────────────────────────────────
# Backward-compatibility aliases for older modules such as core/local_reroute.py
# ─────────────────────────────────────────────────────────────────────────────
def is_reasonable_reroute(original_km, new_km):
    return is_reasonable_detour(original_km, new_km)

def is_reasonable_route(original_km, new_km):
    return is_reasonable_detour(original_km, new_km)

def is_reasonable_motorbike_detour(original_km, new_km):
    return is_reasonable_motorbike_avoid_expressway(original_km, new_km)

def __getattr__(name):
    # Prevent hard crash if an older file imports a historical helper name.
    if name.startswith("is_reasonable"):
        return is_reasonable_detour
    if name.endswith("limit_text"):
        return detour_limit_text
